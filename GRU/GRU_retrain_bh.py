...
def initiate():
    ...
    for epoch in range(500):
        ...
        loss_acc_mse_mae.to_csv('./out_volve_xj/data/loss_acc_mse_mae_.csv', sep=",", index=True)

        torch.save(model.state_dict(), './out_volve_xj/model/Model_volve_xj.pkl')

        csv_train.to_csv('./out_volve_xj/data/rel_pre_train.csv', sep=",", index=True)
        csv_test.to_csv('./out_volve_xj/data/rel_pre_test.csv', sep=",", index=True)

        acc_loss_plot_two(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                          './out_volve_xj/png/loss.png')
        acc_loss_plot_two(loss_acc_mse_mae['train_r2'], loss_acc_mse_mae['test_r2'], 'r2',
                          './out_volve_xj/png/acc.png')
        true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                       './out_volve_xj/png/pre_true_test.png')
        true_test_plot(csv_train['train_depth'], csv_train['train_true'], csv_train['train_pre'], 'train',
                       './out_volve_xj/png/pre_true_train.png')
