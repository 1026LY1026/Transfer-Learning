...
base_path = "./out_volve_xj/test/300_{}"
model_path = "./out_volve_xj/model/Model_volve_xj.pkl"
...
def initiate(model_pre_len):
    ...
    test_r2, test_mse, test_mae, test_epoch_loss, test_true, test_pre, test_depth = test(model,data_dataloader, y_max, y_min, de_max, de_min)
    ...
    print(' r2 =', '{:.6f}'.format(test_r2), ' mse =', '{:.6f}'.format(test_mse), ' mae =', '{:.6f}'.format(test_mae), 'time = ', start)
    ...
    true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                   os.path.join(save_path, "xj2.png"))
...
for model_pre_len in model_pre_len_values:
    ...
    # 加载数据
    data_dataloader, y_max, y_min, de_max, de_min = test_data_load(data_path_well_2)
...
summary_df.to_csv("./out_volve_xj/test/summary_results_xj2.txt", sep="\t", index=True, float_format="%.6f")
