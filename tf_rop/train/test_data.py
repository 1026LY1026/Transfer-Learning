...
def single_data(data_path):
    well_3 = pd.read_csv(data_path)
    data_all = well_3
   #  data_all=well_3[['MD','RPMA','ROPA']]
    # data_all = pd.concat([data_path])
...
def data_load(seq_len, data_path):
    x, y = single_data(data_path)
    len = x.shape[0]
...
def dataset(seq_len,data_path):
    X, Y = data_load(seq_len,data_path)
