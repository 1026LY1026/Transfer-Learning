...
test_loader, test_feature_size, test_out_size = test_data.dataset(seq_len,data_path_well_2)
...
def initiate():
    ...
    for epoch in range(2500):
        ...
        loss_acc_mse_mae.to_csv('tf/train/D/xj_en/loss_acc_mse_mae_2.csv', sep=",", index=True)

       
        torch.save(model.state_dict(), 'tf/train/D/model/Model_xj_enco.pkl')

        acc_loss_plot(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                        'tf/train/D/xj_en/loss_2.png')
        acc_loss_plot(loss_acc_mse_mae['train_acc'], loss_acc_mse_mae['test_acc'], 'acc',
                        'tf/train/D/xj_en/acc_2.png')
