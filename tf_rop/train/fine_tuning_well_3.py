...
# train_loader, train_feature_size, train_out_size = test_data.dataset(seq_len,data_path_well_3)
test_loader, test_feature_size, test_out_size = test_data.dataset(seq_len,data_path_well_1)
...
def initiate():
    ...
    for epoch in range(500):
        ...
        loss_acc_mse_mae.to_csv('tf/train/output/xj/fine_tuning/loss_acc_mse_mae_1.csv', sep=",", index=True)

        if epoch % 1 == 0:
            if epoch >=0:
                torch.save(model.state_dict(), 'tf/train/out_model/xj/Model_volve_well3_1.pkl')
                ...
                csv_train.to_csv('tf/train/output/xj/fine_tuning/rel_pre_train_1.csv', sep=",", index=True)

                csv_test = pd.concat([test_de, test_t, test_p], axis=1)
                csv_test.to_csv('tf/train/output/xj/fine_tuning/rel_pre_test_1.csv', sep=",", index=True)

                acc_loss_plot(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                              'tf/train/output/xj/fine_tuning/loss_1.png')
                acc_loss_plot(loss_acc_mse_mae['train_acc'], loss_acc_mse_mae['test_acc'], 'acc',
                              'tf/train/output/xj/fine_tuning/acc_1.png')
                true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                               'tf/train/output/xj/fine_tuning/test_true_pre_1.png')
