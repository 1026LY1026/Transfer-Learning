...
model.load_state_dict(torch.load('./out_model/volve/Model_volve_10.pkl', map_location=torch.device('cuda')))
...
def initiate():
    ...
    for epoch in range(1000):
        ...
        loss_acc_mse_mae.to_csv('./out_bh_new_fintuning/data/loss_acc_mse_mae_retrain_11510.csv', sep=",", index=True)

      
        torch.save(model.state_dict(), './out_bh_new_fintuning/model/Model_volve_bh_retrain_11510.pkl')
        ...
        csv_train.to_csv('./out_bh_new_fintuning/data/rel_pre_train_retrain_11510.csv', sep=",", index=True)

        csv_test = pd.concat([test_de, test_t, test_p], axis=1)
        csv_test.to_csv('./out_bh_new_fintuning/data/rel_pre_test_retrain_11510.csv', sep=",", index=True)

        acc_loss_plot(loss_acc_mse_mae['train_loss'], loss_acc_mse_mae['test_loss'], 'loss',
                        './out_bh_new_fintuning/png/loss_retrain_11510.png')
        acc_loss_plot(loss_acc_mse_mae['train_acc'], loss_acc_mse_mae['test_acc'], 'acc',
                        './out_bh_new_fintuning/data/acc_retrain_11510.png')
        ...
        true_test_plot(csv_test['test_depth'], csv_test['test_true'], csv_test['test_pre'], 'test',
                        './out_bh_new_fintuning/data/pre_true_test_retrain_11510.png')
